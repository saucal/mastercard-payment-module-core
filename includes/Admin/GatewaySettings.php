...
final class GatewaySettings {
	...
	public function get_settings() {
		...
		$settings = array(
			'enabled'          => array(
				...
				'label'       => __( 'Enable', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no',
			),
			'title'            => array(
				'title'       => __( 'Title', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'type'        => 'text',
				'description' => __( 'The payment method title displayed during checkout.', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'default'     => $this->mpgs_plugin->mpgs_core()->plugin_title(),
				'desc_tip'    => true,
			),
			'description'      => array(
				'title'       => __( 'Description', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'type'        => 'text',
				'description' => esc_html__( 'The description displayed when this payment method is selected.', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'default'     => esc_html__( 'Pay with your Credit/Debit Card', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'desc_tip'    => true,
			),
			'region'           => array(
				'title'   => __( 'Merchant Region', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'type'    => 'select',
			...
			'sandbox'          => array(
				...
				'label'       => __( 'Enable test sandbox mode', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'type'        => 'checkbox',
				'description' => __( 'Place the payment gateway in test mode using test API credentials (real payments will not be taken).', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'default'     => 'no',
				'desc_tip'    => true,
			),
			'merchant_id'      => array(
				'title'       => __( 'Merchant ID', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'type'        => 'text',
				'description' => __( 'This is your merchant profile ID.', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'default'     => '',
			),
			'password'         => array(
				'title'       => __( 'API Password', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'type'        => 'password',
				'description' => __( 'This is your API password.', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'default'     => '',
			),
		);
	...
	private function maybe_add_advanced_settings( $settings ) {
		...
		return array_merge(
			...
			array(
				'payments'             => array(
					'title' => __( 'Payment configurations', $this->mpgs_plugin->mpgs_core()->text_domain() ),
					'type'  => 'title',
				),
				...
				'checkout_mode'        => array(
					'title'   => __( 'Integration mode', $this->mpgs_plugin->mpgs_core()->text_domain() ),
					'type'    => 'select',
					'options' => self::checkout_modes(),
					'default' => 'hosted_session',
				),
				'hosted_checkout_mode' => array(
					'title'             => __( 'Hosted Checkout Mode', $this->mpgs_plugin->mpgs_core()->text_domain() ),
					'type'              => 'select',
					'options'           => array(
						'embedded' => __( 'Embedded', $this->mpgs_plugin->mpgs_core()->text_domain() ),
						'redirect' => __( 'Redirect to Payment Page', $this->mpgs_plugin->mpgs_core()->text_domain() ),
					),
					'default'           => 'embedded',
				...
				'_3d_secure'           => array(
					...
					'type'              => 'checkbox',
					'default'           => 'yes',
					'description'       => __( 'Contact your payment service provider if you need more information.', $this->mpgs_plugin->mpgs_core()->text_domain() ),
					'desc_tip'          => true,
					'class'             => 'conditional-hide',
					'custom_attributes' => array(
						'data-show-rel' => 'checkout_mode',
				...
				'debug'                => array(
					'title'       => __( 'Logging', $this->mpgs_plugin->mpgs_core()->text_domain() ),
					'label'       => __( 'Log debug messages', $this->mpgs_plugin->mpgs_core()->text_domain() ),
					'type'        => 'checkbox',
					'description' => __( 'Save debug messages to the WooCommerce System Status log.', $this->mpgs_plugin->mpgs_core()->text_domain() ),
					'default'     => 'yes',
					'desc_tip'    => true,
				),
			)
	...
	public function payment_regions() {
		...
		return array(
			'ap' => array(
				'name' => __( 'Asia Pacific and Middle East', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'code' => 'ap',
				'url'  => 'https://ap-gateway.mastercard.com',
			),
			'eu' => array(
				'name' => __( 'Europe', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'code' => 'eu',
				'url'  => 'https://eu-gateway.mastercard.com',
			),
			'na' => array(
				'name' => __( 'North America', $this->mpgs_plugin->mpgs_core()->text_domain() ),
				'code' => 'na',
	...
	public function checkout_modes() {
		return array(
			'hosted_session'  => __( 'Hosted Session', $this->mpgs_plugin->mpgs_core()->text_domain() ),
			'hosted_checkout' => __( 'Hosted Checkout', $this->mpgs_plugin->mpgs_core()->text_domain() ),
		);
	}
